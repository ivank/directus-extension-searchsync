...
module.exports = function elasticsearch(config) {
	...
	async function deleteItems(collection) {
		try {
			await axios.post(
				`${config.host}/${indexPrefix}${collection}/_delete_by_query`,
				{
					query: {
						match_all: {},
					},
	...
	async function deleteItem(collection, id) {
		try {
			await axios.delete(`${config.host}/${indexPrefix}${collection}/_doc/${id}`, axiosConfig);
		} catch (error) {
			if (error.response && error.response.status === 404) return;
	...
	async function updateItem(collection, id, data) {
		try {
			await axios.post(`${config.host}/${indexPrefix}${collection}/_doc/${id}`, data, axiosConfig);
		} catch (error) {
			if (error.response) {
